"""Tests for the MoltenSaltAnalyzer class."""
...
def test_select_trajectory_preference(analyzer):
    """Test that a the preferred trajectory is selected."""
...
def test_select_trajectory_only_nvt():
    """Test that the fallback on the other trajectory is used when the preferred is not available."""
    ...
    traj, _ = ana._select_trajectory("npt", T=1100)
    assert traj is ana.trajs_nvt[0]  # type: ignore


def test_init_string_input():
    """Test that paths can be provided as strings."""
    ana = msc.MoltenSaltAnalyzer(
    ...
    assert len(ana.trajs_npt) == 1  # type: ignore
...
def test_compute_thermal_expansion(analyzer):
    ...
    assert np.isclose(thm_exp_coeff, thm_exp_coeff_ref, atol=1e-5), (
...
def test_compute_diffusion_coefficient_arr_fit(analyzer):
    """Test that the diffusion coefficient is computed correctly."""
    diff_coeffs = []
...
def test_rdf_auto_pairs_multiprocessing(analyzer):
    """Test that the multiprocessing for RDF computation doesn't crash."""
    ...
    assert rdf_keys == rdf_keys_ref, f"RDF auto-pairs {rdf_keys} do not match expected {rdf_keys_ref}"


def test_autocorr_fft_known_signal(analyzer):
    """Test that the autocorrelation function is computed correctly."""
    x = np.array([1.0, 2.0, 3.0])
    ac = analyzer._autocorr_fft(x, nmax=3)
    expected = np.array([14 / 3, 4.0, 3.0])
    assert np.allclose(ac, expected), f"Autocorrelation function is {ac} instead of {expected}"
...
def test_invalid_temperature(analyzer):
    """Test that an error is raised when a temperature was not provided during init."""
...
def test_init_mismatched_lengths():
    """Test that an error is raised when the number of trajectory files and temperatures don't match."""
    with pytest.raises(ValueError) as e:
        msc.MoltenSaltAnalyzer(
            traj_files_npt=["a.traj", "b.traj"],
            temperatures_npt=[1100],
        )
    assert "Number of NPT trajectory files and temperatures_npt must match" in str(e.value)


def test_init_missing_temperatures():
    """Test that an error is raised when no temperatures were provided during init."""
    with pytest.raises(ValueError) as e:
...
def test_select_no_trajectory():
    """Test that an error is raised when a trajectory file is attempted to be selected but none were initialized."""
    ana = msc.MoltenSaltAnalyzer()
    with pytest.raises(ValueError) as e:
        ana._select_trajectory("npt", T=1100)
    assert "No trajectory files provided" in str(e.value)


def test_eq_density_invalid_fraction(analyzer):
    """Test that the equilibrium fraction is checked for validity."""
    with pytest.raises(ValueError) as e:
...
def test_invalid_thm_expansion(analyzer, monkeypatch):
    """Test that an error is raised when the thermal expansion is computed with insufficient data."""
...
def test_rdf_no_pairs(analyzer):
    """Test that an error is raised when the RDF is computed but no pairs are specified."""
    with pytest.raises(ValueError) as e:
...
def test_viscosity_nonconstant_timestep():
    """Test that an error is raised when trying to compute the viscosity from a trajectory with a non-constant timestep."""
    ana = msc.MoltenSaltAnalyzer(
        traj_files_nvt=[
            BASE / "test_analyzer_trajectories" / "nvt_NaCl_1200K_nonconstant_timestep.traj",
        ],
